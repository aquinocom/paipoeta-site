...
PROJECT_NAME=pai-poeta
STACK_NAME=paipoeta-com-br
...
stack-start:  ## Local Stack: Start Services
	...
	@echo "Now visit: http://pai-poeta.localhost"
...
acceptance-frontend-image-build:
	...
	@docker build frontend -t luxcas/pai-poeta-frontend:acceptance -f frontend/Dockerfile --build-arg VOLTO_VERSION=$(VOLTO_VERSION)

.PHONY: acceptance-backend-image-build
acceptance-backend-image-build:
	@echo "Build acceptance backend image"
	@docker build backend -t luxcas/pai-poeta-backend:acceptance -f backend/Dockerfile.acceptance --build-arg PLONE_VERSION=$(PLONE_VERSION)
...
acceptance-frontend-container-start:
	@echo "Start acceptance frontend"
	@docker run --rm -p 3000:3000 --name pai-poeta-frontend-acceptance --link pai-poeta-backend-acceptance:backend -e RAZZLE_API_PATH=http://localhost:55001/plone -e RAZZLE_INTERNAL_API_PATH=http://backend:55001/plone -d luxcas/pai-poeta-frontend:acceptance

.PHONY: acceptance-backend-container-start
acceptance-backend-container-start:
	@echo "Start acceptance backend"
	@docker run --rm -p 55001:55001 --name pai-poeta-backend-acceptance -d luxcas/pai-poeta-backend:acceptance
...
acceptance-containers-stop: ## Stop Acceptance containers
	...
	@docker stop pai-poeta-frontend-acceptance
	@docker stop pai-poeta-backend-acceptance
